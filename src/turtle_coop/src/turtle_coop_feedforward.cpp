...
private:
    ...
    void followLeader() {
        ...
        // leader velocities (turtlesim::Pose provides linear_velocity and angular_velocity)
        double leader_lin = leader_pose_.linear_velocity;
        double leader_ang = leader_pose_.angular_velocity;
        double leader_heading = wrapToPi(leader_pose_.theta);

        // feedforward projection: leader velocity component along the line follower->leader
        double rel_heading = wrapToPi(leader_heading - angle_to_leader);
        double v_ff = leader_lin * std::cos(rel_heading);
        double omega_ff = leader_ang;
        ...
        // feedforward gains (tunable)
        const double beta_lin_ff = 0.8;   // fraction of leader linear velocity to add
        const double beta_ang_ff = 0.9;   // fraction of leader angular velocity to add
        ...
        if (distance < dist_deadzone) {
            ...
            // proportional commands with clamping + feedforward
            double linear_cmd = k_linear * distance;
            linear_cmd += beta_lin_ff * v_ff;                 // add feedforward
            linear_cmd = std::clamp(linear_cmd, 0.0, max_linear);

            double angular_cmd = 0.0;
            if (std::fabs(angle_difference) < angle_deadzone) {
                angular_cmd = 0.0;
            } else {
                angular_cmd = k_angular * angle_difference;
            }
            // add angular feedforward and clamp
            angular_cmd += beta_ang_ff * omega_ff;
